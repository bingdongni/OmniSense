"""Ozon Spider - Russian e-commerce platform"""
...
class OzonSpider(BaseSpider):
    """Ozon Spider - Russian online retailer"""

    def __init__(self, headless: bool = True, proxy: Optional[str] = None):
        super().__init__(platform="ozon", headless=headless, proxy=proxy)
    ...
    async def search(self, keyword: str, max_results: int = 20) -> List[Dict[str, Any]]:
        try:
            ...
            results = []
            for item in soup.select("[class*='tile'], [class*='product']")[:max_results]:
                try:
                    results.append({
