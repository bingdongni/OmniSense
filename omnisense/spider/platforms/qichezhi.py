"""Qichezhi (汽车之家) Spider - Auto community platform"""
...
class QichezhiSpider(BaseSpider):
    """Qichezhi Spider - Chinese automotive community"""

    def __init__(self, headless: bool = True, proxy: Optional[str] = None):
        super().__init__(platform="qichezhi", headless=headless, proxy=proxy)
        self.base_url = "https://www.autohome.com.cn"

    async def login(self, username: str, password: str) -> bool:
        try:
            login_url = f"{self.base_url}/beijing/login.html"
    ...
    async def search(self, keyword: str, max_results: int = 20) -> List[Dict[str, Any]]:
        try:
            ...
            results = []
            for item in soup.select(".search-item, [class*='result']")[:max_results]:
                try:
                    results.append({
