"""Pinduoduo (拼多多) Spider - Social e-commerce platform"""
...
class PinduoduoSpider(BaseSpider):
    """Pinduoduo Spider - Chinese social commerce platform"""

    def __init__(self, headless: bool = True, proxy: Optional[str] = None):
        super().__init__(platform="pinduoduo", headless=headless, proxy=proxy)
        self.base_url = "https://www.pinduoduo.com"
    ...
    async def search(self, keyword: str, max_results: int = 20) -> List[Dict[str, Any]]:
        try:
            search_url = f"{self.base_url}/search_result.html?search_key={keyword}"
            await self.navigate(search_url)
            await asyncio.sleep(2)
            html = await self._page.content()
            soup = self.parser.parse_html(html)
            results = []
            for item in soup.select("[class*='goods'], [class*='item']")[:max_results]:
                try:
                    results.append({
                        "title": self.parser.extract_text(item, "[class*='title']"),
                        "price": self.parser.extract_text(item, "[class*='price']"),
                        "sales": self.parser.extract_text(item, "[class*='sales']"),
